...
class QuizReport extends Component {
	...
	componentDidMount() {
		this.setState( {
			isLoaded: true,
			isProVersion:
				wisdm_learndash_reports_editor_script_quiz_reports.is_pro_version_active,
		} );
	}

	render() {
		let body = <div>{ __( 'Loading…', 'learndash-reports-pro' ) }</div>;
		if ( ! this.state.isLoaded ) {
			body = (
				<div className="wisdm-learndash-reports-chart-block wisdm-ld-loading">
			...
			body = (
				<div className="wisdm-learndash-reports-chart-block">
					<div className="wisdm-learndash-reports-quiz-reports">
						<span>
							{ __(
								'This block will be hidden by default & will display the Quiz Reports when quiz reports are requested',
...
registerBlockType( 'wisdm-learndash-reports/quiz-reports', {
	title: __( 'Quiz Reports', 'learndash-reports-pro' ),
	description: __(
		'A table containing a list of the courses',
		'learndash-reports-pro'
	),
