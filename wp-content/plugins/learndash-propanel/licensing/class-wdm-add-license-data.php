    ...
    class WdmAddLicenseData
    {
        ...
        public static function checkLicenseStatus($licenseData, $pluginSlug)
        {
            ...
            if (isset($licenseData->license) && !empty($licenseData->license)) {
                if (isset($licenseData->renew_link) && !empty($licenseData->renew_link)) {
                    $licenseKey = self::getKeyFromURL($licenseData->renew_link);
                    update_option('edd_'. $pluginSlug . '_' . $licenseKey . '_data', $licenseData);
                }
        ...
        /**
         * Gets the key from the URL get parameter & returns
         */
        public static function getKeyFromURL( $url , $key_string = 'edd_license_key') {
			_deprecated_function( __METHOD__, '1.8.2' );
            ...
            if ( ! empty( $url ) ) {
                ...
                $key = isset($params[$key_string])?$params[$key_string]:'';
            }
            return $key;
        }
        ...
        public function activateLicense()
        {
            ...
            if ($licenseKey) {
                ...
                if (is_wp_error($response)) {
                    ...
                    delete_option('edd_'.$this->pluginSlug . '_' . $licenseKey .'_data');
                    return false;
                }

                $licenseData = json_decode(wp_remote_retrieve_body($response));
                update_option('edd_'.$this->pluginSlug . '_' . $licenseKey.'_data', $licenseData);
