 ...
 * or copy this template to yourtheme/uncanny-toolkit/frontend-dashboard/dashboard-template.php.
 *
 * Available Variables:
...
<?php if ( $has_custom_colors ) { ?>
	...
	<style>
		...
		.ultp-dashboard-topic--expanding .ultp-dashboard-lesson__row .ultp-dashboard-lesson__toggle-btn,
		.ultp-dashboard-topic--expanded .ultp-dashboard-lesson__row .ultp-dashboard-lesson__toggle-btn {
		...
		.ultp-dashboard-topic--expanding .ultp-dashboard-lesson__row .ultp-dashboard-lesson__toggle-btn:before,
		.ultp-dashboard-topic--expanded .ultp-dashboard-lesson__row .ultp-dashboard-lesson__toggle-btn:before {
...
<div class="ultp-dashboard">
	<div class="ultp-dashboard-toolbar">
				...
				<?php if ( $has_wp_category_dropdown ) { ?>
					...
					<div class="ultp-dashboard-filter">
						<div class="ultp-dashboard-filter__label">
							<?php esc_attr_e( 'Category', 'uncanny-pro-toolkit' ); ?>
						</div>
						<div class="ultp-dashboard-filter__field">
							<select name="catid">
								<option value="">
									<?php esc_attr_e( 'All categories', 'uncanny-pro-toolkit' ) ?>
								</option>
								...
								<?php foreach ( $wp_categories as $category ) {
									...
									<option value="<?php echo $category->id; ?>" <?php echo $category->is_selected ? 'selected' : ''; ?>>
										<?php printf( esc_attr_x( '%1$s (%2$s)', '%1$s is the category name, %2$s is the number of courses that category has', 'uncanny-pro-toolkit' ), $category->title, $count ); ?>
									</option>
								<?php } ?>
							</select>
						</div>
						...
						<div class="ultp-dashboard-filter__field">
							<select name="course_catid">
								...
								<?php foreach ( $ld_categories as $category ) { ?>
									<option value="<?php echo $category->id; ?>" <?php echo $category->is_selected ? 'selected' : ''; ?>>
										<?php printf( esc_attr_x( '%1$s (%2$s)', '%1$s is the category name, %2$s is the number of courses that category has', 'uncanny-pro-toolkit' ), $category->title, $category->number_of_courses ); ?>
									</option>
								<?php } ?>
							</select>
						</div>
		...
		<div class="ultp-dashboard-actions">
			<div class="ultp-dashboard-btn ultp-dashboard-btn--expand-all">
				<?php esc_attr_e( 'Expand all', 'uncanny-pro-toolkit' ); ?>
			</div>
			<div class="ultp-dashboard-btn ultp-dashboard-btn--collapse-all">
				<?php esc_attr_e( 'Collapse all', 'uncanny-pro-toolkit' ); ?>
			</div>
		</div>
	</div>
	...
	<div class="ultp-dashboard-box">
		<div class="ultp-dashboard-courses">
			...
			if( ! isset($user_enrolled_courses) ){
			...
			if ( ! empty( $courses ) ) { ?>
						...
						<div class="ultp-dashboard-course__row">
							...
							<div class="ultp-dashboard-course__details">
								...
								<div class="ultp-dashboard-course__right">
									...
									<?php if ( $course->has_ceu ) { ?>
										<?php if ( ! empty( $course->ceus_available ) ) { ?>

											<div class="ultp-dashboard-course__ceus">
												<div class="ultp-dashboard-course__ceus-numbers">
													<span class="ultp-dashboard-course__ceus-earned">
														<?php echo $course->ceus_earned; ?>
													</span>
													<span class="ultp-dashboard-course__separator">/</span>
													<span class="ultp-dashboard-course__ceus-available">
														<?php echo $course->ceus_available; ?>
													</span>
												</div>
												<div class="ultp-dashboard-course__ceus-text">
													<?php echo $course->ceus_name; ?>
												</div>
											</div>
									...
									if ( in_array( $course->status, array(
										'not_started',
										'not-started'
									), true ) ) {
										// Then show "Start" button
										$action->text = esc_attr__( 'Start', 'uncanny-pro-toolkit' );
										$action->url  = $course->url;

										if ( ! in_array( $course->id, $user_enrolled_courses, true ) ) {
											$action->text = esc_attr__( 'View', 'uncanny-pro-toolkit' );
										}

									} elseif ( in_array( $course->status, array(
										'in_progress',
										'in-progress'
									), true ) ) {
										if ( $course->has_resume_url ) {
											// "Resume" button
											$action->text = esc_attr__( 'Resume', 'uncanny-pro-toolkit' );
											$action->url  = $course->resume_url;
										}
									} elseif ( $course->status == 'completed' ) {
										if ( $course->has_certificate ) {
											// "Certificate" button
											$action->text = esc_attr__( 'Certificate', 'uncanny-pro-toolkit' );
									...
									<?php if ( ! empty( $action->text ) && $user_id === wp_get_current_user()->ID ) { ?>

										<div class="ultp-dashboard-course__action <?php echo esc_attr($course->status); ?>">
									...
									<div class="ultp-dashboard-course__progress">
										<div class="ultp-dashboard-course__progress-sizer">
											<?php printf( esc_attr__( '%s complete', 'uncanny-pro-toolkit' ), '100%' ); ?>
										</div>
										<div class="ultp-dashboard-course__progress-percentage">
											<?php printf( esc_attr__( '%s complete', 'uncanny-pro-toolkit' ), sprintf( '%s%%', $course->progress ) ); ?>
										</div>
						...
						<div class="ultp-dashboard-course__content">
							...
							<?php if ( $course->has_lessons ) { ?>
								<div class="ultp-dashboard-course__lessons">
									<div class="ultp-dashboard-sections">
										...
										<?php foreach ( $course->sections as $section ) { ?>
											...
											// Set array with the extra classes we're going to
											// add to the section div
											$section_css_classes = [];

											// Check if we have to expand it on page load
											/*if ( $expanded_on_load ) {
												$section_css_classes[] = 'ultp-dashboard-section--expanded';
											} else {
												$section_css_classes[] = 'ultp-dashboard-section--collapsed';
											}*/
											// Expand by default
											$section_css_classes[] = 'ultp-dashboard-section--expanded';
											...
											<div class="ultp-dashboard-section <?php echo implode( ' ', $section_css_classes ); ?>">

												<?php if ( ! empty( $section->title ) ) { ?>

													<div class="ultp-dashboard-section__header">
														<?php /* <div class="ultp-dashboard-quizzes__header-toggle-btn"></div> */ ?>
														<div class="ultp-dashboard-section__header-title">
												...
												<div class="ultp-dashboard-section__content">
																				...
																				<?php if ( ! $lesson->is_available ) { ?>
																					...
																					<div class="ultp-dashboard-lesson__available-on">
																						...
																						<div class="ultp-dashboard-lesson__available-on-date">
																							<?php echo $lesson->available_on_message; ?>
																						</div>
																					</div>
																				<?php } ?>
																			</div>
																		...
																		<div class="ultp-dashboard-lesson__topics">
																			<div class="ultp-dashboard-topics">
																				...
																				<?php foreach ( $lesson->topics as $topic ) { ?>
																					...
																					// Check if we have to expand it on page load
																					if ( $expanded_on_load && $lesson->has_quizzes ) {
																						$topic_css_classes[] = 'ultp-dashboard-topic--expanded';
																					} else {
																						$topic_css_classes[] = 'ultp-dashboard-topic--collapsed';
																					}

																					// Add class if it has quizzes
																					if ( $topic->has_quizzes ) {
																						$topic_css_classes[] = 'ultp-dashboard-topic--has-quizzes';
																					}
																					...
																					<div class="ultp-dashboard-topic <?php echo implode( ' ', $topic_css_classes ); ?>"
																						...
																						<div class="ultp-dashboard-topic__row">
																							<div class="ultp-dashboard-topic__toggle-btn"></div>
																						...
																						<div class="ultp-dashboard-topic__content">
																							<?php if ( $topic->has_quizzes ) { ?>

																								<div class="ultp-dashboard-topic__quizzes">
																									...
																									<div class="ultp-dashboard-quizzes <?php echo implode( ' ', $quizzes_css_classes ); ?>">
																										<div class="ultp-dashboard-quizzes__header">
																											...
																											<div class="ultp-dashboard-quizzes__header-title">
																												<?php echo esc_attr( LearnDash_Custom_Label::get_label( 'quizzes' ) ); ?>
																											</div>
																										</div>
																														...
																														<div class="ultp-dashboard-quiz__details">
																															...
																															<div class="ultp-dashboard-quiz__right">
																																<div class="ultp-dashboard-quiz__action">
																																	...
																																	<?php if ( $quiz->has_certificate && $user_id === wp_get_current_user()->ID ) { ?>

																																		<a href="<?php echo $quiz->certificate_url; ?>"
																																		   target="_blank"
																																		   class="ultp-dashboard-btn ultp-dashboard-btn--certificate">
																																			<?php esc_attr_e( 'Certificate', 'uncanny-pro-toolkit' ); ?>
																																		</a>

																																	<?php } ?>

																																	<?php if ( $quiz->has_statistics  && learndash_get_setting( $quiz->id, 'statisticsOn' )  ) { ?>
																																		...
																																		<a class="ultp-dashboard-btn ultp-dashboard-btn--statistics user_statistic"
																																		   ...
																																		   data-nonce="<?php echo $quiz->nonce; ?>"
																																		   data-ref_id="<?php echo intval( $quiz->statistic_ref_id ); ?>"
																																		   href="#">
																																			<?php esc_attr_e( 'Statistics', 'uncanny-pro-toolkit' ); ?>
																																		</a>
																																...
																																<?php if ( null !== $quiz->passed ): ?>
																																	<div class="ultp-dashboard-quiz__score">
																																		<div class="ultp-dashboard-quiz__score-label">
																																			<?php
																																			if ( $quiz->passed ) {
																																				printf( esc_attr__( '%s Passed', 'uncanny-pro-toolkit' ), sprintf( '%s%%', $quiz->score ) );
																																			} else {
																																				printf( esc_attr__( '%s Failed', 'uncanny-pro-toolkit' ), sprintf( '%s%%', $quiz->score ) );
																																			}
																																			?>
																																		</div>
																																	</div>
														...
														<div class="ultp-dashboard-quiz__details">
															...
															<div class="ultp-dashboard-quiz__right">
																<div class="ultp-dashboard-quiz__action">
																	...
																	<?php 
																	if ( $quiz->has_statistics && learndash_get_setting( $quiz->id, 'statisticsOn' )  ) { ?>
				...
				<?php } ?>
			<?php }else{
				if ( isset( $atts['no_courses_message'] ) && ! empty( $atts['no_courses_message'] ) ) { ?>
				<div class="uo-dashboard no-courses-message"><?php echo esc_html($atts['no_courses_message']); ?></div>
				<?php } ?>
			<?php } ?>
