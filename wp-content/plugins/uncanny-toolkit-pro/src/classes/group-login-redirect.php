...
class GroupLoginRedirect extends toolkit\Config implements toolkit\RequiredFunctions {
	...
	public static function get_details() {
		$module_id = 'group-login-redirect';
	...
	public static function render_metabox( $post ) {
		...
		$icon             = LEARNDASH_LMS_PLUGIN_URL . 'assets/images/question.png';
		// Echo out the field
		ob_start();
		?>
        ...
        <div class="sfwd_input " id="sfwd-group_login_redirect">
			<span class="sfwd_option_label" style="text-align:right;vertical-align:top;">
				<a class="sfwd_help_text_link" style="cursor:pointer;"
                   title="<?php esc_attr_e( 'Click for Help!', 'uncanny-pro-toolkit' ) ?>"
                   onclick="toggleVisibility('uo-redirect-link');">
					<img src="<?php echo $icon; ?>">
					<label class="sfwd_label textinput"><?php esc_attr_e( 'Redirect members on login', 'uncanny-pro-toolkit' ) ?></label>
				...
				<div class="sfwd_help_text_div" style="display:none" id="uo-redirect-link">
					<label class="sfwd_help_text"><?php esc_attr_e( 'Specify the URL (relative or absolute) that members will be redirected to upon log in.', 'uncanny-pro-toolkit' ) ?></label>
				</div>
			</span>
        ...
        <div class="sfwd_input " id="sfwd-group_login_redirect">
			<span class="sfwd_option_label" style="text-align:right;vertical-align:top;">
				<a class="sfwd_help_text_link" style="cursor:pointer;"
                   title="<?php esc_attr_e( 'Click for Help!', 'uncanny-pro-toolkit' ) ?>"
                   onclick="toggleVisibility('uo-redirect-priority');">
					<img src="<?php echo $icon; ?>">
					<label class="sfwd_label textinput"><?php esc_attr_e( 'Redirect priority', 'uncanny-pro-toolkit' ) ?></label>
				</a></span><span class="sfwd_option_input">
				<div class="sfwd_option_div">
					<input name="_uo_group_redirect_priority" type="text" size="57"
                           value="<?php echo $redirect_priority; ?>">
				</div>
				<div class="sfwd_help_text_div" style="display:none" id="uo-redirect-priority">
					<label class="sfwd_help_text"><?php esc_attr_e( 'Specify a priority for this redirect. (optional) Default is 99.', 'uncanny-pro-toolkit' ) ?></label>
				</div>
			</span>
            <p style="clear:left"></p>
        </div>
	 ...
	 * @return null
	 */
	public static function save_redirect_settings( $post_id, $post ) {

		if ( filter_has_var( INPUT_POST, 'custom_nonce' ) && filter_has_var( INPUT_POST, '_uo_group_redirect_link' ) && filter_has_var( INPUT_POST, '_uo_group_redirect_priority' ) ) {
			// Add nonce for security and authentication.
			$nonce_name   = filter_has_var( INPUT_POST, 'custom_nonce' ) ? filter_input( INPUT_POST, 'custom_nonce' ) : '';
			...
			update_post_meta( $post_id, '_uo_group_redirect_link', filter_input( INPUT_POST, '_uo_group_redirect_link' ) );
			update_post_meta( $post_id, '_uo_group_redirect_priority', filter_input( INPUT_POST, '_uo_group_redirect_priority' ) );
