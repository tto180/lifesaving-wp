...
/**
 * Class uncanny_learndash_groups\Tools\System\Report
 *
...
class Report {
	 ...
	 * @return int
	 */
	public function ulgm_string_to_num( $size ) {
		$l   = substr( $size, - 1 );
	...
	public function get_environment_info( $fields = array( 'environment' ) ) {
		...
		if ( function_exists( 'curl_version' ) ) {
			...
			$curl_version = __( 'cURL installed but unable to retrieve version.', 'uncanny-learndash-groups' );
		}

		// WP memory limit.
		$wp_memory_limit = $this->ulgm_string_to_num( WP_MEMORY_LIMIT );
		if ( function_exists( 'memory_get_usage' ) ) {
			$wp_memory_limit = max( $wp_memory_limit, $this->ulgm_string_to_num( @ini_get( 'memory_limit' ) ) ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged
		}
		...
		if ( $enable_remote_post ) {
			$post_response_code = get_transient( 'ulgm_test_remote_post' );
			...
			if ( false === $post_response_code || is_wp_error( $post_response_code ) ) {
				$response = wp_safe_remote_post(
					...
					array(
						'timeout'     => 10,
						'user-agent'  => 'Uncanny-Groups-Learndash/' . UNCANNY_GROUPS_VERSION,
						'httpversion' => '1.1',
						'body'        => array(
				...
				set_transient( 'ulgm_test_remote_post', $post_response_code, HOUR_IN_SECONDS );
			}
		...
		if ( $enable_remote_get ) {
			$get_response_code = get_transient( 'ulgm_test_remote_get' );
			...
			if ( false === $get_response_code || is_wp_error( $get_response_code ) ) {
				...
				set_transient( 'ulgm_test_remote_get', $get_response_code, HOUR_IN_SECONDS );
			}
		...
		$environment      = array(
			...
			'php_post_max_size'      => $this->ulgm_string_to_num( ini_get( 'post_max_size' ) ),
	...
	public function output_tables_info() {
		...
		$db_report = new DB_Report();

		$missing_tables = $db_report->verify_base_tables();
		...
		<strong style="color:#a00;">
			...
			echo esc_html(
				...
				// translators: Comma seperated list of missing tables.
					__( 'Missing base tables: %s. Some Uncanny Groups functionality may not work as expected.', 'uncanny-learndash-groups' ),
	...
	public function get_database_info() {
		...
		if ( defined( 'DB_NAME' ) ) {
			...
			// Uncanny Groups tables to check existence of.
			$core_tables = (object) apply_filters(
				'ulgm_database_tables',
				(object) array(
					'group_codes'   => 'ulgm_group_codes',
					'group_details' => 'ulgm_group_details',
				)
			);
			...
			/**
			 * Organize Uncanny Groups and other plugin tables separately for display purposes later.
			 *
			 * To ensure we include all Uncanny Groups tables, even if they do not exist, pre-populate the Uncanny Groups array with all the tables.
			 */
			$tables = array(
				'uncanny_groups' => array_fill_keys( $core_tables, false ),
				'other'          => array(),
			);
			...
			foreach ( $database_table_information as $table ) {
				...
				$table_type = in_array( $table->name, $core_tables, true ) ? 'uncanny_groups' : 'other';
		...
		return array(
			'ulgm_database_version'           => get_option( 'ulgm_database_version' ),
			'ulgm_database_available_version' => UNCANNY_GROUPS_DB_VERSION,
	...
	public function output_plugins_info( $plugins ) {
		foreach ( $plugins as $plugin ) {
			if ( ! empty( $plugin['name'] ) ) {
				...
				if ( ! empty( $plugin['url'] ) ) {
					$plugin_name = '<a href="' . esc_url( $plugin['url'] ) . '" aria-label="' . esc_attr__( 'Visit plugin homepage', 'uncanny-learndash-groups' ) . '" target="_blank">' . $plugin_name . '</a>';
				}
				...
				<tr>
					...
					<td>
						...
						printf( esc_html__( 'by %s', 'uncanny-learndash-groups' ), esc_html( $plugin['author_name'] ) );
