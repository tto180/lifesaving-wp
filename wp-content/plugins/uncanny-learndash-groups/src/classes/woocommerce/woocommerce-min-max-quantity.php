...
class WoocommerceMinMaxQuantity {
	...
	* Get the total quantity of the product available in the cart.
	 ...
	 * @return void
	 */
	public function woocommerce_cart_item_price_refresh( $price, $cart_item, $cart_item_key ) {
		...
		if ( isset( $cart_item['product_id'] ) && 'yes' === $fixed_price ) {
			return get_woocommerce_currency_symbol() . floatval( get_post_meta( $cart_item['product_id'], '_price', true ) ) . ' / ' . $_subscription_period . ' / ' . SharedFunctions::get_per_seat_text( 2 );
		}

		return get_woocommerce_currency_symbol() . floatval( get_post_meta( $cart_item['product_id'], '_price', true ) ) . ' / ' . $_subscription_period;
	}
}
