...
class LearndashGroupsPostEditAdditions {
	...
	public function render_metabox_for_woo( $post ) {
		...
		<ul>
			<li>
				<?php echo sprintf( '<h4>%s: <a href="%s">#%d %s %s</a> <i>(Status: %s)</i></h4>', esc_html__( 'Main order', 'uncanny-learndash-groups' ), get_edit_post_link( $order_id ), $order_id, $order->get_billing_first_name(), $order->get_billing_last_name(), $order->get_status() ); ?>
			</li>
		</ul>
