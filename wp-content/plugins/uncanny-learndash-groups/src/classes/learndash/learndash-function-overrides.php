...
class LearndashFunctionOverrides {
	...
	/**
	 * Get all the users enrolled in a group and its children groups if hierarchy is enabled.
	 *
	 * @param int $group_id
	 *
	 * @return array
	 */
	public static function learndash_maybe_get_hierarchial_group_user_ids( $group_id ) {
		$group_id = absint( $group_id );
		// Bail early if group id is not set
		if ( empty( $group_id ) ) {
			return array();
		...
		// Check if hierarchy setting is enabled.
		if ( ! SharedFunctions::is_hierarchy_enabled( $group_id ) ) {
			return self::learndash_get_groups_user_ids( $group_id );
		}
		...
		$user_ids = self::learndash_get_groups_user_ids( $group_id );
		foreach ( $group_children as $child_group_id ) {
			$group_user_ids = self::learndash_get_groups_user_ids( $child_group_id );
			if ( ! empty( $group_user_ids ) ) {
				$user_ids = array_merge( $user_ids, $group_user_ids );
			}
		}

		return ! empty( $user_ids ) ? array_unique( $user_ids ) : array();
	}
...
AND meta_value = %d",
	...
	public static function learndash_get_administrators_group_ids( $user_id = 0 ) {
		...
		if ( empty( $user_id ) ) {
			$user_id = get_current_user_id();
			// Bail if the current user is not able to be determined.
			if ( empty( $user_id ) ) {
				return array();
			}
		}
...
WHERE um.meta_key LIKE '%s'
  AND (
	...
	public static function ulgm_get_group_users( $group_id = 0, $args = array(), $user_id = false ) {
		...
		$ms_query = '';
		if ( is_multisite() ) {
			if ( is_main_site() || get_current_network_id() !== get_current_blog_id() ) {
