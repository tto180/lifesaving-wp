...
class Group_Management_Helpers {
	...
	public static function add_invite_user( $user_data, $do_not_send_emails = false, $counter = false, $is_api = true ) {
		...
		$user_login_column_exists = key_exists( 'user_login', $user_data );
		...
		if ( $user_login_column_exists && ! empty( $user_data['user_login'] ) && mb_strlen( $user_data['user_login'] ) > 60 ) {
			if ( ! $counter ) {
		...
		$sanitized_user_name = '';
		if ( $user_login_column_exists && ! empty( $user_data['user_login'] ) ) {
			$sanitized_user_name = sanitize_user( $user_data['user_login'] );
			if ( strtolower( $sanitized_user_name ) !== strtolower( $user_data['user_login'] ) ) {
				if ( ! $counter ) {
		...
		if ( $user_login_column_exists && ! empty( $sanitized_user_name ) && false === $email_exists ) {
		...
		if ( $email_exists || $username_exists ) {
			...
			if ( isset( $user_data['user_login'] ) && ! empty( $user_data['user_login'] ) && strtolower( $user_object->user_login ) !== strtolower( $user_data['user_login'] ) ) {
		...
		if ( $user_login_column_exists && isset( $user_data['user_login'] ) && empty( $user_data['user_login'] ) ) {
