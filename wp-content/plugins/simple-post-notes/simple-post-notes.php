...
class SPNotes {
	...
	function add_quick_edit_field( $column_name, $post_type ) {
		...
		$label = wp_filter_nohtml_kses( sanitize_text_field(
			html_entity_decode( $this->settings['notes_label'], ENT_QUOTES, 'UTF-8' )
		) );

		$placeholder = isset( $this->settings['notes_placeholder'] ) ? $this->settings['notes_placeholder'] : '';
		$placeholder = wp_filter_nohtml_kses( sanitize_text_field(
			html_entity_decode( $placeholder, ENT_QUOTES, 'UTF-8' )
		) );
	...
	public function output_column( $column, $post_id ) {
		...
		if ( $column == 'spnote' ) {
			...
			if ( $note ) {
				echo '<div id="spnote-' . esc_attr( $post_id ) . '">';
	...
	public function metabox( $post ) {
		...
		$placeholder = wp_filter_nohtml_kses( sanitize_text_field(
			html_entity_decode( $this->settings['notes_placeholder'], ENT_QUOTES, 'UTF-8' )
		) );
	...
	public function save_bulkedit_note() {
		...
		if ( ! empty( $post_ids ) && is_array( $post_ids ) && ! empty( $spnote ) ) {
			foreach( $post_ids as $post_id ) {
				update_post_meta( $post_id, '_spnote', sanitize_textarea_field( $spnote ) );
			}
		}
	...
	public function settings_post_type_field() {
		...
		echo '<select multiple="multiple" name="spnotes_settings[post_types][]" id="post_types" class="chosen-select" style="width: 300px;">';
			...
			foreach ( get_post_types( array( 'public' => true ), 'objects' ) as $post_type ) {
				...
				echo '<option value="' . esc_attr( $post_type->name ) . '" ' . esc_attr( $selected ) . '>' . esc_attr( $post_type->labels->name ) . '</option>';
	...
	public function enqueue_scripts_and_styles( $hook ) {
		...
		wp_enqueue_script( 'spnotes/chosen', SPNOTES . 'assets/chosen/chosen.jquery.min.js', array(
			'jquery'
		), filemtime( SPNOTES_DIR . 'assets/chosen/chosen.jquery.min.js' ), true );

		wp_enqueue_script( 'spnotes/admin', SPNOTES . 'assets/admin.js', array(
			'jquery',
			'spnotes/chosen'
		), filemtime( SPNOTES_DIR . 'assets/admin.js' ), true );


		// enqueue styles

		wp_enqueue_style( 'spnotes/chosen', SPNOTES . 'assets/chosen/chosen.min.css', array(), filemtime( SPNOTES_DIR . 'assets/chosen/chosen.min.css' ) );
