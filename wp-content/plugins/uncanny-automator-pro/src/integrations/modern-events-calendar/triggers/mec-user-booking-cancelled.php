...
class MEC_USER_BOOKING_CANCELLED {
	...
	public function __construct() {
		...
		$this->trigger_code = 'MECUSERBOOKCANCEL';
		$this->trigger_meta = 'MECUSERCANCELBOOK';
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			/* translators: The Event or `Any Event` */
				esc_attr__( "A user's booking of {{an event:%1\$s}} is cancelled", 'uncanny-automator-pro' ),
				$this->trigger_code
			),
			'select_option_name'  => esc_attr__( "A user's booking of {{an event}} is cancelled", 'uncanny-automator-pro' ),
			'action'              => 'mec_booking_canceled',
			//mec_booking_canceled
			'priority'            => 99,
			'accepted_args'       => 1,
			'validation_function' => array(
				$this,
				'mec_user_booking_cancelled',
			),
	 ...
	 *
	 * @param $booking_id The accepted post id of `mec_user_booking_cancelled`
	 *     action.
	 *
	 * @return void.
	 */
	public function mec_user_booking_cancelled( $booking_id ) {
							...
							foreach ( $args as $result ) {
								...
								if ( true === $result['result'] && $result['args']['trigger_id'] && $result['args']['trigger_log_id'] ) {
									...
									$event_book_id_action_meta = array(
										...
										'meta_value'     => sprintf( 'EVENT_BOOKING_%d_CANCELLED', $booking_id ),
									);
