...
/**
 * Class MEC_USER_BOOKING_CONFIRMED
 *
...
class MEC_USER_BOOKING_CONFIRMED {
	...
	public function __construct() {
		...
		$this->trigger_code = 'MECUSERBOOKCONFIRMED';
		$this->trigger_meta = 'MECUSERCONFIRMEDINGBOOK';
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			/* translators: The Event or `Any Event` */
				esc_attr__( "A user's booking of {{an event:%1\$s}} is confirmed", 'uncanny-automator-pro' ),
				$this->trigger_code
			),
			'select_option_name'  => esc_attr__( "A user's booking of {{an event}} is confirmed", 'uncanny-automator-pro' ),
			'action'              => 'mec_booking_confirmed',
			'priority'            => 99,
			'accepted_args'       => 2,
			'validation_function' => array( $this, 'mec_user_booking_confirmed' ),
			'options'             => array(
				$events_options,
			),
		);
	 ...
	 *
	 * @param $booking_id The accepted post id of `mec_booking_confirmed` action.
	 * @param $mode Mode of confirming booking
	 *
	 * @return void.
	 */
	public function mec_user_booking_confirmed( $booking_id, $mode ) {
						...
						if ( $args ) {
							...
							foreach ( $args as $result ) {
								...
								if ( true === $result['result'] && $result['args']['trigger_id'] && $result['args']['trigger_log_id'] ) {
									...
									$run_number = Automator()->get->trigger_run_number( $result['args']['trigger_id'], $result['args']['trigger_log_id'], $user->ID );
