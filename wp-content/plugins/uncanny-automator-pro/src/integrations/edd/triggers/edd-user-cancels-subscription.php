...
/**
 * Class EDD_USER_CANCELS_SUBSCRIPTION
 *
 * @package Uncanny_Automator_Pro
 */
class EDD_USER_CANCELS_SUBSCRIPTION extends \Uncanny_Automator\Recipe\Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'EDDR_SUBSCRIPTION_CANCELS' );
		$this->set_trigger_meta( 'EDDR_PRODUCTS' );
		$this->set_is_pro( true );
		$this->set_sentence( sprintf( esc_attr_x( 'A user cancels a subscription to {{a download:%1$s}}', 'Easy Digital Downloads - Recurring Payments', 'uncanny-automator' ), $this->get_trigger_meta() ) );
		$this->set_readable_sentence( esc_attr_x( 'A user cancels a subscription to {{a download}}', 'Easy Digital Downloads - Recurring Payments', 'uncanny-automator' ) );
