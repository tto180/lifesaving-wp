...
class BB_POSTATOPIC {
	...
	public function define_action() {
		...
		$action = array(
			...
			'priority'           => 10,
			'accepted_args'      => 1,
			'execution_function' => array( $this, 'post_a_forum_topic' ),
			...
			array(
				'options_group' => array(
					$this->action_meta => array(
						Automator()->helpers->recipe->bbpress->options->list_bbpress_forums( esc_attr__( 'Forum', 'uncanny-automator-pro' ), $this->action_meta, false ),
						Automator()->helpers->recipe->field->text_field( 'BBTOPICTITLE', esc_attr__( 'Topic title', 'uncanny-automator-pro' ), true, 'text', '', false ),
						Automator()->helpers->recipe->field->text_field( 'BBTOPICCONTENT', esc_attr__( 'Topic content', 'uncanny-automator-pro' ), true, 'textarea' ),
					),
				),
			)
		);
	...
	public function post_a_forum_topic( $user_id, $action_data, $recipe_id, $args ) {
		...
		$forum_id       = Automator()->parse->text( $action_data['meta']['BBFORUMTOPIC'], $recipe_id, $user_id, $args );
		$topic_title    = Automator()->parse->text( $action_data['meta']['BBTOPICTITLE'], $recipe_id, $user_id, $args );
		$topic_title    = do_shortcode( $topic_title );
		$topic_content  = Automator()->parse->text( $action_data['meta']['BBTOPICCONTENT'], $recipe_id, $user_id, $args );
		...
		if ( ! bbp_check_for_duplicate(
			...
			$action_data['complete_with_errors'] = true;
			Automator()->complete_action( $user_id, $action_data, $recipe_id, __( "Duplicate discussion detected; it looks as though you've already said that!", 'uncanny-automator-pro' ) );
		...
		if ( ! bbp_check_for_blacklist( $anonymous_data, $topic_author, $topic_title, $topic_content ) ) {
			$action_data['complete_with_errors'] = true;
			Automator()->complete_action( $user_id, $action_data, $recipe_id, __( 'Sorry, Your discussion cannot be created at this time.', 'uncanny-automator-pro' ) );
