...
/**
 * Class WC_VARIATION_PRODUCT_OUT_OF_STOCK
 *
 * @pacakge Uncanny_Automator_Pro
 */
class WC_VARIATION_PRODUCT_OUT_OF_STOCK extends Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'WC_VARIATION_OUT_OF_STOCK' );
		$this->set_trigger_meta( 'WC_VARIATION' );
		$this->set_is_pro( true );
		$this->set_trigger_type( 'anonymous' );
		$this->set_helper( new Woocommerce_Pro_Helpers() );
		$this->set_sentence( sprintf( esc_attr_x( '{{A product variation:%1$s}} is out of stock', 'WooCommerce', 'uncanny-automator-pro' ), $this->get_trigger_meta() ) );
		$this->set_readable_sentence( esc_attr_x( '{{A product variation}} is out of stock', 'WooCommerce', 'uncanny-automator-pro' ) );
