...
/**
 * Class WCM_IS_NOT_MEMBER
 *
 * @package Uncanny_Automator_Pro
 */
class WCM_IS_NOT_MEMBER extends Action_Condition {
	...
	public function define_condition() {
		...
		/* translators: Token */
		$this->name         = __( 'The user does not have an active membership of {{a specific plan}}', 'uncanny-automator-pro' );
		$this->code         = 'IS_NOT_MEMBER';
		$this->dynamic_name = sprintf(
			/* translators: A token matches a value */
			esc_html__( 'The user does not have an active membership of {{a specific plan:%1$s}}', 'uncanny-automator-pro' ),
			'MEMBERSHIP'
		);
	...
	public function evaluate_condition() {
		...
		if ( ! empty( $is_member ) ) {
			if ( $membership_id < 0 ) {
				// User has Any Active memberships.
				$message = __( 'User has membership plan(s).', 'uncanny-automator-pro' );
			} else {
				// User has specific membership.
				$message = sprintf(
