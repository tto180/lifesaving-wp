...
/**
 * Class WP_USERNAME_EXISTS
 *
 * @package Uncanny_Automator_Pro
 */
class WP_USERNAME_EXISTS extends Action_Condition {
	...
	public function define_condition() {
		...
		$this->integration  = 'WP';
		$this->name         = __( 'A user with {{a username}} exists on the site', 'uncanny-automator-pro' );
		$this->code         = 'USERNAME_EXISTS';
		$this->dynamic_name = sprintf(
		/* translators: Email address */
			esc_html__( 'A user with {{a username:%1$s}} exists on the site', 'uncanny-automator-pro' ),
			'USER_LOGIN'
		);
	...
	public function evaluate_condition() {
		...
		if ( false === boolval( $condition_met ) ) {
			/* translators: Email address */
			$message = sprintf( __( 'The username (%s) does not exist on the site', 'uncanny-automator-pro' ), $username );
			$this->condition_failed( $message );
		}
	}
}
