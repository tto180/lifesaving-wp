...
/**
 * Class WP_POST_META_KEY_NOT_EXISTS
 *
 * @package Uncanny_Automator_Pro
 */
class WP_POST_META_KEY_NOT_EXISTS extends Action_Condition {
	...
	public function define_condition() {
		...
		$this->integration  = 'WP';
		$this->name         = __( '{{A specific}} meta key does not exist for {{a post}}', 'uncanny-automator-pro' );
		$this->code         = 'POST_META_KEY_NOT_EXISTS';
		$this->dynamic_name = sprintf(
		/* translators: the meta key */
			esc_html__( '{{A specific:%1$s}} meta key does not exist for {{a post:%2$s}}', 'uncanny-automator-pro' ),
			'METAKEY',
			'POST'
		);
		$this->is_pro        = true;
	...
	public function evaluate_condition() {
		...
		if ( true === (bool) $condition_met ) {
			$message = __( 'Post has the required meta key: ', 'uncanny-automator-pro' ) . $meta_key;
