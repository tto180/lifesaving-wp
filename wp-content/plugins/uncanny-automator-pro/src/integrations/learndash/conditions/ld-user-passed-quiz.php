...
/**
 * Class LD_USER_PASSED_QUIZ
 *
 * @package Uncanny_Automator_Pro
 */
class LD_USER_PASSED_QUIZ extends Action_Condition {
	...
	public function define_condition() {
		...
		$this->integration = 'LD';
		/*translators: Token */
		$this->name = __( 'The user has passed {{a quiz}}', 'uncanny-automator-pro' );
		$this->code = 'PASSED_A_QUIZ';
		// translators: A token matches a value
		$this->dynamic_name  = sprintf( esc_html__( 'The user has passed  {{a quiz:%1$s}}', 'uncanny-automator-pro' ), 'QUIZ' );
		$this->is_pro        = true;
	...
	public function evaluate_condition() {
		...
		if ( 1 !== absint( $quiz_progress['pass'] ) ) {
			$message = __( 'User has failed the quiz ', 'uncanny-automator-pro' ) . $this->get_option( 'QUIZ_readable' );
			$this->condition_failed( $message );
		}
	}
