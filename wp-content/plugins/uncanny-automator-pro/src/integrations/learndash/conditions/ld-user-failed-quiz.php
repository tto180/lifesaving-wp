...
/**
 * Class LD_USER_FAILED_QUIZ
 *
 * @package Uncanny_Automator_Pro
 */
class LD_USER_FAILED_QUIZ extends Action_Condition {
	...
	public function define_condition() {
		...
		$this->integration = 'LD';
		/*translators: Token */
		$this->name = __( 'The user has failed {{a quiz}}', 'uncanny-automator-pro' );
		$this->code = 'FAILED_A_QUIZ';
		// translators: A token matches a value
		$this->dynamic_name  = sprintf( esc_html__( 'The user has failed  {{a quiz:%1$s}}', 'uncanny-automator-pro' ), 'QUIZ' );
		$this->is_pro        = true;
	...
	public function evaluate_condition() {
		...
		$quiz_progress = array_pop( $quiz_progress );
		// if empty, user has not attempted
		if ( empty( $quiz_progress ) ) {
			$message = __( 'User has not attempted the quiz ', 'uncanny-automator-pro' ) . $this->get_option( 'QUIZ_readable' );
			$this->condition_failed( $message );

			return;
		}
		if ( 1 === absint( $quiz_progress['pass'] ) ) {
			$message = __( 'User has passed the quiz ', 'uncanny-automator-pro' ) . $this->get_option( 'QUIZ_readable' );
			$this->condition_failed( $message );
		}
	}
