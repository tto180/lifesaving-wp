...
/**
 * Class COMPLETED_A_COURSE
 *
 * @package Uncanny_Automator_Pro
 */
class LD_COMPLETED_A_COURSE extends Action_Condition {
	...
	public function define_condition() {
		...
		$this->integration = 'LD';
		/*translators: Token */
		$this->name = __( 'The user has completed {{a course}}', 'uncanny-automator-pro' );
		$this->code = 'COMPLETED_A_COURSE';
		// translators: A token matches a value
		$this->dynamic_name  = sprintf( esc_html__( 'The user has completed {{a course:%1$s}}', 'uncanny-automator-pro' ), 'COURSE' );
		$this->is_pro        = true;
	...
	public function evaluate_condition() {
		...
		if ( false === (bool) $has_completed ) {
			...
			$message = __( 'User has not completed ', 'uncanny-automator-pro' ) . $this->get_option( 'COURSE_readable' );
