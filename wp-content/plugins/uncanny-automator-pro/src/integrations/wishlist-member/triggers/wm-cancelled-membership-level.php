...
/**
 * Class WM_CANCELLED_MEMBERSHIP_LEVEL
 *
...
class WM_CANCELLED_MEMBERSHIP_LEVEL {
	...
	public function setup_trigger() {
		...
		$this->set_trigger_code( 'CANCELLEVEL' );
		$this->set_trigger_meta( 'WMMEMBERSHIPLEVELS' );
		$this->set_is_login_required( true );
		$this->set_is_pro( true );
		/* Translators: Trigger sentence */
		$this->set_sentence( sprintf( esc_html__( "A user's access to {{a membership level:%1\$s}} is cancelled", 'uncanny-automator-pro' ), $this->get_trigger_meta() ) );
		/* Translators: Trigger sentence */
		$this->set_readable_sentence( esc_html__( "A user's access to {{a membership level}} is cancelled", 'uncanny-automator-pro' ) ); // Non-active state sentence to show
		$this->add_action( 'wishlistmember_cancel_user_levels' );
