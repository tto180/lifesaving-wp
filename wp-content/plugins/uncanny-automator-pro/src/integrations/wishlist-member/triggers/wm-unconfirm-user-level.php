...
class WM_UNCONFIRM_USER_LEVEL {
	...
	public function setup_trigger() {
		...
		$this->set_trigger_code( 'UNCONFIRMUSERLEVEL' );
		$this->set_trigger_meta( 'WMMEMBERSHIPLEVELS' );
		$this->set_is_login_required( true );
		$this->set_is_pro( true );
		/* Translators: Trigger sentence */
		$this->set_sentence( sprintf( esc_html__( 'A user is unconfirmed for {{a membership level:%1$s}}', 'uncanny-automator-pro' ), $this->get_trigger_meta() ) );
		/* Translators: Trigger sentence */
		$this->set_readable_sentence( esc_html__( 'A user is unconfirmed for {{a membership level}}', 'uncanny-automator-pro' ) ); // Non-active state sentence to show
		$this->add_action( 'wishlistmember_unconfirm_user_levels' );
