...
class GIVE_DONATIONAMOUNT {
	...
	public function __construct() {
		$this->trigger_code = 'DONATIONAMOUNT';
	...
	public function define_trigger() {
		$trigger = array(
			...
			'sentence'            => sprintf( esc_attr__( 'A user makes a donation via {{a form:%1$s}} for an amount {{greater than, less than, or equal to:%2$s}} {{an amount:%3$s}}', 'uncanny-automator' ), $this->trigger_meta, 'NUMBERCOND', 'DONATEDAMOUNT' ),
			/* translators: Logged-in trigger - GiveWP */
			'select_option_name'  => esc_attr__( 'A user makes a donation via {{a form}} for an amount {{greater than, less than, or equal to}} {{an amount}}', 'uncanny-automator-pro' ),
		...
		foreach ( $recipes as $recipe_id => $recipe ) {
			foreach ( $recipe['triggers'] as $trigger ) {
				...
				if ( isset( $required_form[ $recipe_id ] ) && isset( $required_form[ $recipe_id ][ $trigger_id ] ) ) {
					...
					if ( intval( '-1' ) === intval( $required_form[ $recipe_id ][ $trigger_id ] ) && Automator()->utilities->match_condition_vs_number( $required_condition[ $recipe_id ][ $trigger_id ], $required_amount[ $recipe_id ][ $trigger_id ], $amount ) ) {
						$matched_recipe_ids[] = array(
							'recipe_id'  => $recipe_id,
							'trigger_id' => $trigger_id,
						);
					} //Add where option is set to Specific Form
					elseif ( $required_form[ $recipe_id ][ $trigger_id ] == $give_form_id && Automator()->utilities->match_condition_vs_number( $required_condition[ $recipe_id ][ $trigger_id ], $required_amount[ $recipe_id ][ $trigger_id ], $amount ) ) {
						$matched_recipe_ids[] = array(
							'recipe_id'  => $recipe_id,
							'trigger_id' => $trigger_id,
						);
