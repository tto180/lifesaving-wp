...
/**
 * Class ANON_INTEGRATELYWEBHOOKS
 */
class ANON_INTEGRATELYWEBHOOKS {
	...
	public function __construct() {
		$this->trigger_code = 'ANON_INTEGRATELYWEBHOOKS';
	...
	public function define_trigger() {
		...
		$trigger = array(
			...
			'support_link'        => Automator()->get_author_support_link( $this->trigger_code, 'integration/integrately/' ),
			'is_pro'              => true,
			'integration'         => self::$integration,
			'code'                => $this->trigger_code,
			/* translators: Anonymous trigger - Integrately */
			'sentence'            => sprintf( __( 'Receive data from Integrately {{webhook:%1$s}}', 'uncanny-automator-pro' ), 'WEBHOOK_DATA' ),
			/* translators: Anonymous trigger - Integrately */
			'select_option_name'  => __( 'Receive data from Integrately {{webhook}}', 'uncanny-automator-pro' ),
			'action'              => 'automator_pro_run_webhook',
			'priority'            => 10,
			'accepted_args'       => 3,
			'type'                => 'anonymous',
			'validation_function' => array( $this, 'run_integrately_webhook' ),
	 ...
	 * @param $recipe
	 */
	public function run_integrately_webhook( $param, $recipe, $request ) {
