...
/**
 * @package Uncanny_Automator\Integrations\Loopable_Csv\Triggers
 *
 * @since 6.0
 */

namespace Uncanny_Automator\Integrations\Loopable_Csv\Triggers;

use Uncanny_Automator\Automator_Status;
use Uncanny_Automator\Integrations\Loopable_Csv\Helpers\Loopable_Csv_Helpers;
use Uncanny_Automator_Pro\Integrations\Loopable_Csv\Tokens\Loopable\Trigger\Csv_Items;
...
/**
 *
 * Loopable_Csv_Trigger
 *
 * @package Uncanny_Automator\Integrations\Loopable_Csv\Triggers
 *
 */
class Loopable_Csv_Trigger extends \Uncanny_Automator\Recipe\Trigger {
	...
	protected function setup_trigger() {
		...
		$this->set_trigger_code( 'TRIGGER_LOOPABLE_CSV_CODE' );
		$this->set_trigger_meta( 'TRIGGER_LOOPABLE_CSV_META' );
		...
		$this->set_loopable_tokens(
			array(
				'TRIGGER_LOOPABLE_CSV_ITEMS' => Csv_Items::class,
			)
		);
	...
	public function register_hooks() {
		...
		$closure = function( $item, $recipe_id ) {
			...
			if ( 'TRIGGER_LOOPABLE_CSV_META' !== $requesting_meta ) {
				return;
			}
			...
			$config = array(
				'loopable_id' => 'TRIGGER_LOOPABLE_CSV_ITEMS',
				'type'        => 'TRIGGER_TOKEN',
	...
	public function options() {
		return Loopable_Csv_Helpers::make_fields( $this->get_trigger_meta() );
	}
