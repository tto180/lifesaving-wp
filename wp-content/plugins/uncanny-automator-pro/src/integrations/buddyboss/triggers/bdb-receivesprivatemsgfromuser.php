...
class BDB_RECEIVESPRIVATEMSGFROMUSER {
	 ...
	 * @var string
	 */
	const TRIGGER_CODE = 'BDB_RECEIVESPRIVATEMSGFROMUSER';
	 ...
	 * @var string
	 */
	const TRIGGER_META = 'BDB_RECEIVESPRIVATEMSGFROMUSER_META';
	...
	public function setup_trigger() {
		...
		$this->set_sentence( sprintf( esc_html__( 'A user receives a private message from {{a user:%1$s}}', 'uncanny-automator-pro' ), $this->get_trigger_meta() ) );

		$this->set_readable_sentence(
			esc_html__( 'A user receives a private message from {{a user}}', 'uncanny-automator-pro' )
		);
		...
		if ( null !== $this->bdb_tokens ) {
			...
			$this->set_tokens( ( new Bdb_Pro_Tokens( false ) )->receive_msg_to_user_tokens() );
	...
	public function parse_additional_tokens( $parsed, $args, $trigger ) {
		...
		return $this->bdb_tokens->receive_msg_to_usr_hydrate_tokens( $parsed, $args, $trigger );
