 ...
 * Class BDB_USER_HAS_VALUE_IN_XFIELD
 *
 * @package Uncanny_Automator_Pro
 */
class BDB_USER_HAS_VALUE_IN_XFIELD extends Loop_Filter {
	...
	public function setup() {
		...
		$this->set_meta( 'BDB_USER_HAS_VALUE_IN_XFIELD' );
		$this->set_sentence( esc_html_x( 'A user has {{a value}} in {{an Xprofile field}}', 'Filter sentence', 'uncanny-automator-pro' ) );
		$this->set_sentence_readable(
			sprintf(
			/* translators: Filter sentence */
				esc_html_x( 'A user has {{a value:%1$s}} in {{an Xprofile field:%2$s}}', 'Filter sentence', 'uncanny-automator-pro' ),
				'FIELD_VALUE',
				$this->get_meta()
			)
		);
	...
	/**
	 * @param array{BDB_USER_HAS_VALUE_IN_XFIELD:string,FIELD_VALUE:string} $fields
	 *
	 * @return int[]
	 */
	public function retrieve_users_field_value( $fields ) {
		...
		$field       = $fields['BDB_USER_HAS_VALUE_IN_XFIELD'];
		...
		foreach ( $all_user_ids as $user_id ) {
			...
			if ( true === Automator()->helpers->recipe->buddyboss->options->pro->check_field_value( $user_xprofile_field_value, $field_value ) ) {
				$users[] = $user_id;
			}
		}
