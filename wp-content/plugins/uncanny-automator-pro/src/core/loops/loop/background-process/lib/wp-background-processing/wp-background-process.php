<?php
namespace Uncanny_Automator_Pro\Loops\Loop\Background_Process\Lib\WP_Background_Processing;
...
abstract class WP_Background_Process extends WP_Async_Request {
	...
	public function maybe_handle() {
		...
		$validated = automator_pro_loop_auth_token()->validate_token( $token, Entity_Actions::AUTH_ACTION );

		// This should never return false. Unless the request didn't come from a loopback request.
		if ( false === $validated ) {
			automator_log( 'Loops failed to start. Invalid authentication token.', 'Loops error', true, 'loops-error' );
			return $this->maybe_wp_die();
		}
	...
	public function schedule_cron_healthcheck( $schedules ) {
		...
		if ( 1 === $interval ) {
			...
			$display = sprintf( __( 'Every %d Minutes' ), $interval ); //phpcs:ignore WordPress.WP.I18n.MissingTranslatorsComment
		}
